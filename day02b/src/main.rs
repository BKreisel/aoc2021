...
impl Submarine {
    pub fn new() -> Submarine {
        Submarine {depth: 0, position: 0, aim: 0}
    }

    pub fn take_action(&mut self, action: &Action) {
        match action.direction {
            Direction::Up => {self.aim -= action.magnitute as i32},
            Direction::Down => {self.aim += action.magnitute as i32},
            Direction::Forward => {
                self.position += action.magnitute as i32;
                self.depth += self.aim * action.magnitute as i32;
            },
        }
    }
...
mod tests {
    ...
    #[test]
    fn test() {
        assert_eq!(run(include_str!("../test.txt")), 900);
    }
}